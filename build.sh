...
SERVICE_NAME=justice-league-parking-service
VERSION=1.0.$BUILD_NUMBER
DOCKER_IMAGE=$DOCKER_REGISTRY/$SERVICE_NAME:$VERSION

sudo $(aws ecr get-login --no-include-email --region cn-northwest-1)

sudo docker build -t justice-league-parking-service .
sudo docker tag justice-league-parking-service:latest $DOCKER_REGISTRY/$SERVICE_NAME:latest
sudo docker tag justice-league-parking-service:latest $DOCKER_IMAGE
